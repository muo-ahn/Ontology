...
from services.dummy_dataset import build_report, default_summary
...
async def run_pipeline(
    ...
    image_path = resolved_path or payload.file_path or f"/tmp/{caption_response.image_id}.png"
    case_id = (entry or {}).get("case_id")
    if not case_id:
        case_id = f"C_{caption_response.image_id}" if not payload.case_id else payload.case_id
    timings: dict[str, int] = {"vlm_ms": int(raw_vlm.get("latency_ms", 0))}
    ...
    if mode == PipelineMode.V:
        output_text = caption_response.caption
    elif mode == PipelineMode.V_L:
        llm_prompt = CAPTION_TO_SUMMARY_PROMPT.format(caption=caption_response.caption)
    ...
    elif mode == PipelineMode.V_G_L:
        report_payload = build_report(
            image_id=caption_response.image_id,
        ...
        graph_context = await fetch_image_context(caption_response.image_id, neo4j)
    ...
    return PipelineResponse(
        ...
        findings=caption_response.findings,
        output=output_text,
        vlm_model=caption_response.model,
